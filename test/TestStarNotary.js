...
// YOUR TEST IS BROKEN.  I AM COMMENTING IT OUT.
//     Error: Returned error: VM Exception while processing transaction: 
// Transaction's maxFeePerGas (0) is less than the block's baseFeePerGas (7) 
...
it('lets user2 buy a star and decreases its balance in ether', async () => {
    ...
    let user2 = accounts[2];
    let starId = 5;
    let starPrice = web3.utils.toWei(".11", "ether");
    let balance = web3.utils.toWei(".15", "ether");
    await instance.createStar('awesome star', starId, { from: user1 });
    await instance.putStarUpForSale(starId, starPrice, { from: user1 });
    let balanceOfUser1BeforeTransaction = await web3.eth.getBalance(user2);
    const balanceOfUser2BeforeTransaction = await web3.eth.getBalance(user2);
    //error begins here (Gas fee is too low)
...
it('can add the star name and star symbol properly', async () => {
    ...
    assert.equal(name, "BOSCO STAR");
    assert.equal(symbol, "BOSCO");
});
